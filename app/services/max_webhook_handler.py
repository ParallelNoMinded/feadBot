"""
Service for handling MAX webhook requests.
...
class MaxWebhookHandler(BaseService):
    """Handles MAX webhook processing with clean separation of concerns."""
    ...
    async def handle_webhook(
        ...
        Args:
            ...
            secret_token: MAX secret token for validation
            adapter: MAX adapter instance
    ...
    async def _validate_and_parse(
        ...
        logger.info("max.webhook.received", payload=payload)

        # Verify secret
        if not require_api_key(secret_token):
    ...
    async def _handle_user_registration(
        self, msg: IncomingMessage, state: InMemoryState, adapter: MaxAdapter
        ...
        if msg.callback_id and msg.text and msg.text.startswith("HOTEL_"):
            return None
        ...
        active_registration = state.get_registration("max", msg.user_id)
    ...
    async def _process_message_context(self, msg: IncomingMessage, state: InMemoryState) -> Tuple[Dict, Dict]:
        ...
        if sp_early:
            # Preserve existing context before processing
            existing_context = (msg.payload or {}).get("_context", {})
            msg.payload = process_sp_early(sp_early, msg)
            # Merge existing context if it was set (e.g., from MaxAdapter)
            if existing_context:
